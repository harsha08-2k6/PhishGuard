...
- `../models/xgboost_phish.pkl`: saved full-data champion model; with `--skip-xgboost`, this is a Random Forest fallback artifact and must not be presented as XGBoost
...
Classic exact `SVC(kernel="rbf")` is not computationally practical for 235,795 rows. The SVM comparison therefore uses a 50-component `Nystroem(kernel="rbf")` map followed by calibrated `LinearSVC`. This is an explicit scalable approximation of the RBF decision function, and it should be named as such in publication tables.
